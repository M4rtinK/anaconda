...
from pyanaconda.modules.common.constants.services import BOSS, TIMEZONE, LOCALIZATION, SECURITY, \
    USER, SERVICES, STORAGE, NETWORK
from pyanaconda.modules.common.constants.objects import DISK_INITIALIZATION, FIREWALL
...
class Firewall(RemovedCommand):
    ...
    def execute(self, storage, ksdata, instClass):
        ...
        # default, do nothing.
        if firewall_proxy.UseSystemDefaults:
        ...
        if firewall_proxy.FirewallEnabled == False:
            args += ["--disabled"]
        else:
            args += ["--enabled"]

        if "ssh" not in firewall_proxy.EnabledServices and "ssh" not in firewall_proxy.DisabledServices \
                and "22:tcp" not in firewall_proxy.EnabledPorts:
            args += ["--service=ssh"]
