...
def give_the_system_a_purpose(sysroot, role, sla, usage, addons):
    ...
    # as the values can contain white space ween need to make sure the
    # values passed to arguments are all quoted
    if role:
        args.extend(["set-role", '"{}"'.format(role)])

    if sla:
        args.extend(["set-sla", '"{}"'.format(sla)])

    if usage:
        args.extend(["set-usage", '"{}"'.format(usage)])

    if addons:
        addon_args = ["add", '"addons"']
        for addon in addons:
            addon_args.append('"{}"'.format(addon))
        args.extend(addon_args)

    if args:
        syspurpose_sysroot_path = os.path.join(sysroot, "syspurpose")
