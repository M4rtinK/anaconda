...
class SetRootPasswordTask(Task):
    ...
    def __init__(self, sysroot, password, crypted, locked):
        ...
        :param bool crypted: is the root password is already encrypted ?
...
class CreateUsersTask(Task):
    ...
    def _create_users(self):
        for user_data in self._user_data_list:
            ...
            try:
                users.create_user(username=user_data.name, password=user_data.password,
                                  is_crypted=user_data.is_crypted, lock=user_data.lock,
                                  homedir=user_data.homedir,
                                  uid=uid, gid=gid, groups=user_data.groups,
                                  shell=user_data.shell, gecos=user_data.gecos,
                                  root=self._sysroot)
