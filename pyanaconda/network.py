...
def dumpMissingDefaultIfcfgs():
    ...
    for devname in nm.nm_devices():
        ...
        try:
            ...
            log.debug("dumping ifcfg file for default autoconnection on %s", devname)
        except nm.SettingsNotFoundError:
                ...
                if "Generated by parse-kickstart" in f.read():
                    ...
                    if network_data.activate:
                        ...
                        if con_uuid != nm.nm_device_active_con_uuid(dev_name):
                            ...
                            log.debug("kickstart - reactivating device %s with %s", dev_name, con_uuid)
                            try:
                                nm.nm_activate_device_connection(dev_name, con_uuid)
                            except nm.UnknownConnectionError:
                                log.warning("kickstart - can't activate connection %s on %s",
                                            con_uuid, dev_name)
                    continue
