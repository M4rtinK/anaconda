...
def doInstall(storage, payload, ksdata, instClass):
    ...
    def run_pre_install():
        ...
        subscription_module = SUBSCRIPTION.get_observer()
        subscription_module.connect()
        role = subscription_module.proxy.Role
        sla = subscription_module.proxy.SLA
        usage = subscription_module.proxy.Usage
        addons = subscription_module.proxy.Addons
        if any((role, sla, usage, addons)):
